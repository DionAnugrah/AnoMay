...
<style>
    ...
    .live-banner {
        ...
        gap: 16px;
        flex-wrap: wrap;
        border: 0.5px solid rgba(255,127,17,0.2);
    ...
    .live-dot {
        ...
        border-radius: 50%;
        background: #ff7f11;
        box-shadow: 0 0 8px rgba(255,127,17,0.6);
    }
    ...
    .rank-table td { padding: 9px 10px; border-bottom: 0.5px solid #f9fafb; color: #374151; }
    ...
    .bar-mini {
        height: 5px;
        border-radius: 3px;
        background: linear-gradient(90deg, #ff7f11, #ffab5e);
        min-width: 4px;
        transition: width 0.6s ease;
    }
    .nama-cell { font-weight: 500; color: #1a1a2e; }
    .omset-cell { font-weight: 600; color: #374151; }

    /* ── Profit Card ── */
    .profit-card {
        background: linear-gradient(135deg, #ff7f11 0%, #ff9a3c 100%);
        border-radius: 14px;
        padding: 1.25rem 1.5rem;
        margin-bottom: 1.5rem;
        display: flex;
        ...
        gap: 16px;
        flex-wrap: wrap;
        box-shadow: 0 4px 16px rgba(255,127,17,0.25);
    }
...
<div class="dash">
    ...
    <div class="live-banner">
        ...
        <div class="live-stats">
            <div class="live-stat">
                <div class="live-stat-val" id="live-aktif">—</div>
                <div class="live-stat-label">Aktif Keliling</div>
            </div>
            <div class="live-stat">
                <div class="live-stat-val" id="live-idle">—</div>
                <div class="live-stat-label">Idle</div>
            </div>
            <div class="live-stat">
                <div class="live-stat-val" id="live-total">—</div>
                <div class="live-stat-label">Total Penjual</div>
            </div>
        </div>
    ...
    <a href="/boss/live-map" class="map-placeholder">
        ...
        <div class="map-icon">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ff7f11" stroke-width="2">
...
<script>
    ...
    async function loadSales() {
        try {
            const res = await fetch('/boss/sales', {
                headers: {
    ...
    async function loadProfit() {
        try {
            const res = await fetch('/boss/profit', {
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Accept': 'application/json'
                }
            ...
            console.error('Gagal load profit:', e);
        }
    }
    ...
    async function loadPerformance() {
        try {
            const res = await fetch('/boss/performance', {
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Accept': 'application/json'
                }
                ...
                tbody.innerHTML = penjual.map((p, i) => {
                    ...
                    return `
                        <tr>
                            ...
                            <td class="nama-cell">${p.name}</td>
                            <td class="omset-cell">${rupiahFull(p.total_setoran)}</td>
                            <td>${p.total_porsi_terjual.toLocaleString('id-ID')} pcs</td>
                            <td style="min-width:90px">
                                <div class="bar-mini" style="width:${porsi}%"></div>
                            </td>
                        </tr>
                    `;
                }).join('');
            ...
            document.getElementById('rank-tbody').innerHTML =
                '<tr><td colspan="5"><div class="error-state">Gagal memuat data.</div></td></tr>';
        }
    }
    ...
    async function loadLiveMap() {
        try {
            ...
            console.error('Gagal load live map:', e);
        }
    }
    ...
    function buildBarChart(penjual) {
        ...
        chartPerforma = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Total Setoran',
                    data: penjual.map(p => p.total_setoran),
                    backgroundColor: penjual.map((_, i) => colors[i % colors.length]),
                    borderRadius: 8,
                    borderSkipped: false,
                }]
            },
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: { label: c => ' ' + rupiahFull(c.raw) }
                    }
                },
                scales: {
                    x: {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const now = new Date();
        document.getElementById('tgl-now').textContent = now.toLocaleDateString('id-ID', {
